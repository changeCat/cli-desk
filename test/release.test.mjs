...
test('sidebar shows the version as the brand subtitle and local storage notice inside new chat',()=>{
  assert.doesNotMatch(html,/你的本地 AI 工作伙伴/);
  assert.match(html,/<div class="brand-title">CLI Desk<\/div><small id="version"/);
  assert.match(html,/<button id="new-chat"[\s\S]*?记录保存在此电脑[\s\S]*?<\/button>/);
